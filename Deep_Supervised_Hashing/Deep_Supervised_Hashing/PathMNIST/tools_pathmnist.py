...
class MyPathMNIST(PathMNIST):
    def __getitem__(self, index):
        img, target = self.data[index], self.targets[index]
        img = Image.fromarray(img.numpy(), mode='L')  # 'L' mode for grayscale
        img = self.transform(img)
        target = np.eye(9, dtype=np.int8)[target]  # Assuming 9 classes for PathMNIST
        return img, target, index
...
def pathmnist_dataset(config):
    batch_size = config["batch_size"]

    train_size = 10000
    test_size = 718

    # Adjust train_size and test_size if necessary
    ...
    train_dataset = PathMNIST(root='./pathmnist/', split='train', transform=transform, download=True)
    test_dataset = PathMNIST(root='./pathmnist/', split='test', transform=transform, download=True)
    database_dataset = PathMNIST(root='./pathmnist/', split='val', transform=transform, download=True)

    # Concatenate data and labels
    X_train, L_train = train_dataset.data, train_dataset.targets
    X_test, L_test = test_dataset.data, test_dataset.targets
    X_database, L_database = database_dataset.data, database_dataset.targets

    # Shuffle and split the data
    train_index, test_index, database_index = [], [], []
    for label in range(10):
        index = torch.where(L_train == label)[0]
        perm = torch.randperm(index.size(0))

        train_index.extend(index[perm[:train_size]])
        test_index.extend(index[perm[train_size:train_size + test_size]])
        database_index.extend(index[perm[train_size + test_size:]])

    # Create datasets using indices
    train_dataset = torch.utils.data.TensorDataset(X_train[train_index], L_train[train_index])
    test_dataset = torch.utils.data.TensorDataset(X_test[test_index], L_test[test_index])
    database_dataset = torch.utils.data.TensorDataset(X_database[database_index], L_database[database_index])
    ...
    return train_loader, test_loader, database_loader, len(train_index), len(test_index), len(database_index)
...
def get_data(config):
    if "cifar" in config["dataset"]:
        ...
        return pathmnist_dataset(config)
